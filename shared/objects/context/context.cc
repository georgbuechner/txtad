...
bool Context::AddAttribute(const std::string& key, std::string initial_value = "") {
  if (_attributes.count(key) > 0) {
    spdlog::get(txtad::LOGGER)->debug("Context::AddAttribute: attribute {} already exists", key);
    return false;
  }
...
void Context::AddListener(std::shared_ptr<Listener> listener) {
  if (_event_manager) {
    ...
    spdlog::get(txtad::LOGGER)->debug("Context::AddListener: event_manager does not exist for listener {}", listener);
  }
}
