...
std::vector<TestCase> test_parser::LoadTestCases(const std::string& game_id) {
  util::Logger()->info("parser:LoadTestCases: path {}", txtad::GamesPath() + game_id + "/" + txtad::GAME_TESTS);
  std::vector<TestCase> test_cases;
  if (auto j_test_cases = util::LoadJsonFromDisc(txtad::GamesPath() + game_id + "/" + txtad::GAME_TESTS)) {
